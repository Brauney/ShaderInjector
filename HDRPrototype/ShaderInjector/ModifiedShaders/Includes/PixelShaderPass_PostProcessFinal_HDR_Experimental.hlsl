...
#define AUTO_EXPOSURE_MAX_EV           1.5
#define AUTO_EXPOSURE_COMPENSATION_EV -0.25
...
// HDR Gran Turismo 7 mode. This must use InitializeAsHDR(), not the SDR helper
// ApplyTonemap_GranTurismo7(), because the latter deliberately maps to sRGB.
#define TONEMAP_GRAN_TURISMO_7

// IMPORTANT: GT7 is currently the only option below with an HDR-calibrated
// implementation. The other LibraryTonemaps operators produce SDR-normalized
// display-linear values. They compile, but enabling one without an HDR output
// wrapper generally confines the scene to roughly the first 100 nits and does
// not make a correct BT.2020/PQ display transform.

// First-pass defaults for a 42-inch LG C2. These are calibration controls, not
// display detection: tune the paper-white value after looking at the image.
#define HDR_PAPER_WHITE_NITS 200.0f
#define HDR_UI_WHITE_NITS    250.0f
#define HDR_PEAK_NITS        700.0f
//#define TONEMAP_AGX
...
#if defined(HDR_COMPOSITE_SDR_THREE_LAYERS)
...
struct PixelOutput
{
	...
	#if defined(HDR_DLSS_FG_OUTPUTS)
		// DLSS Frame Generation uses an MRT permutation of PostProcessFinal.
		// Target 1 carries the PQ-encoded scene before the SDR composite is
		// applied. Target 2 carries the encoded composite delta and opacity
		// reconstructed from targets 0 and 1.
		float4 ColorWithoutComposite : SV_Target1;
		float4 CompositeDelta : SV_Target2;
	#endif
};
...
float3 SampleHDRColorConversionLUT(float3 linearSceneColor)
{
...
// The GT7 library uses a linear framebuffer convention where 1.0 equals
// REFERENCE_LUMINANCE (100 nits). Initialise its HDR curve in that same
// convention, then expose a paper-white calibration control for this display.
float3 ApplyTonemap_GranTurismo7HDR(float3 linearSceneColor)
{
    GT7ToneMapping tm = (GT7ToneMapping)0;
    InitializeAsHDR(tm, HDR_PEAK_NITS);

    const float paperWhiteScale = HDR_PAPER_WHITE_NITS / REFERENCE_LUMINANCE;
    return ApplyToneMapping(tm, max(linearSceneColor, 0.0f.xxx) * paperWhiteScale);
}

static const float3x3 BT709ToBT2020Mat =
{
...
float3 ComposeHDRUI(float3 sceneBT2020Linear100Nits, float4 uiLinear)
{
    // The original HDR DXIL converts the sampled SDR UI through sRGB and a
    // gamma-2.2 working curve before its BT.709 -> BT.2020 matrix. Reproduce
    // that behaviour rather than sending the UI through the scene tonemapper.
    float3 uiGamma22 = pow(max(LinearToSRGB(saturate(uiLinear.rgb)), 0.0f.xxx), 2.2f.xxx);
    float3 uiBT2020Linear100Nits = mul(BT709ToBT2020Mat, uiGamma22) * (HDR_UI_WHITE_NITS / REFERENCE_LUMINANCE);

    // Equivalent to the alpha-squared HDR overlay roll-off visible in the
    // captured shader. It avoids treating an SDR overlay as a 700-nit scene.
    float alpha = saturate(uiLinear.a);
    float alphaSquared = alpha * alpha;
    float3 uiWhiteInSceneUnits = HDR_UI_WHITE_NITS / REFERENCE_LUMINANCE;
    float3 sceneRelativeToUI = sceneBT2020Linear100Nits / max(uiWhiteInSceneUnits, 1.0e-4f.xxx);
    float3 sceneRollOff = (1.0f.xxx + sceneRelativeToUI * alphaSquared) / (1.0f.xxx + sceneRelativeToUI);

    return max(sceneBT2020Linear100Nits * alpha * sceneRollOff + uiBT2020Linear100Nits, 0.0f.xxx);
}

float4 SampleCompositeSDRUI(float2 compositeUV)
{
	float4 composite = CompositeSDRTexture.SampleLevel(View_SharedBilinearClampedSampler, compositeUV, 0.0f);

	#if defined(HDR_COMPOSITE_SDR_THREE_LAYERS)
		// This permutation is used by menu/pause rendering. The captured DXIL
		// treats alpha as the amount of the layer underneath that remains
		// visible, rather than conventional straight-alpha opacity.
		float4 background = CompositeSDRBackgroundTexture.SampleLevel(View_SharedBilinearClampedSampler, compositeUV, 0.0f);
		float4 foreground = CompositeSDRForegroundTexture.SampleLevel(View_SharedBilinearClampedSampler, compositeUV, 0.0f);

		float4 stacked;
		stacked.rgb = composite.rgb + composite.a * background.rgb;
		stacked.a = composite.a * background.a;
		stacked.rgb = foreground.rgb + foreground.a * stacked.rgb;
		stacked.a *= foreground.a;
		return stacked;
	#else
		return composite;
	#endif
...
{
    // This helper remains only for optional experimental tonemappers. It is
    // not used by the HDR baseline's main path.
    return PQToLinear100Nits(SampleHDRColorConversionLUT(linearSceneColor));
}
...
{
    // R10G10B10A2_UNORM needs 10-bit, not SDR 8-bit, output dither.
    float dither = TriangularBlueNoise(uniformNoise) * (1.0f / 1023.0f);
	bool safelyInsideRangeR = abs(encodedColor.r * 2.0f - 1.0f) < (1021.0f / 1023.0f);
	bool safelyInsideRangeG = abs(encodedColor.g * 2.0f - 1.0f) < (1021.0f / 1023.0f);
	bool safelyInsideRangeB = abs(encodedColor.b * 2.0f - 1.0f) < (1021.0f / 1023.0f);
...
PixelOutput main(PixelInput input)
{
	...
	//tonemapping! this is about taking our wide color/brightness range image and compressing it down into SDR/HDR for display
		...
		// SDR's preserve-grade path round-trips through the SDR-only t5 LUT and
		// is therefore not meaningful for the captured HDR permutation.
		// The GT7 implementation works in linear Rec.2020; the game scene input
		// is Rec.709 before the original HDR conversion LUT.
		float3 tonemapOutput = ApplyTonemap_GranTurismo7HDR(mul(BT709ToBT2020Mat, sceneColor));
		...
		#endif
	#else // ORIGINAL GAME HDR COLOR HANDLING
		// Keep the scene in linear HDR units until the SDR UI has been composed.
		// The final BT.709-PQ -> BT.2020-PQ conversion happens below.
		float3 tonemapOutput = sceneColor;
	#endif

	// GT7 output is already linear BT.2020 in the 100-nit framebuffer
	// convention. Add the separately transformed SDR UI in that same domain.
	float4 uiColor = SampleCompositeSDRUI(compositeUV);
	float3 sceneBT2020Linear100Nits = tonemapOutput;
	float3 compositedLinear = ComposeHDRUI(sceneBT2020Linear100Nits, uiColor);

	// HDR10 swapchain output: PQ encoded, BT.2020 primaries.
	float3 encodedOutput = Linear100NitsToPQ(compositedLinear);
	...
	float3 ditheredOutput = ApplyOutputDither(encodedOutput, blueNoise);
    output.Color = float4(ditheredOutput, 0.0f);

	#if defined(HDR_DLSS_FG_OUTPUTS)
		// The captured FG permutations expose the scene without the SDR
		// composite as a second PQ render target. Apply the same tonemapper
		// and dither to this path so generated frames do not mix differently
		// tonemapped scene and UI buffers.
		float3 encodedSceneWithoutComposite = Linear100NitsToPQ(sceneBT2020Linear100Nits);
		float3 ditheredSceneWithoutComposite = ApplyOutputDither(encodedSceneWithoutComposite, blueNoise);
		output.ColorWithoutComposite = float4(ditheredSceneWithoutComposite, 0.0f);

		// Match the captured DXIL in encoded PQ space. Composite alpha is the
		// transmittance of the scene underneath, so one minus alpha is the
		// opacity written to the extracted composite target.
		float compositeTransmittance = uiColor.a;
		float3 compositeDelta = max(
			ditheredOutput - ditheredSceneWithoutComposite * compositeTransmittance,
			0.0f.xxx);
		float compositeOpacity = max(1.0f - compositeTransmittance, 0.0f);
		output.CompositeDelta = float4(compositeDelta, compositeOpacity);
	#endif
